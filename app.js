...
async function downloadMonthlyReport(){
  ...
  try{
    ...
    // Fetch monthly data
    var monthlyData=DATA.monthly||await gasGet('monthly');
    var rows=(monthlyData.rows||[]);
    var ALL_MONTHS=['JANUARY','FEBRUARY','MARCH','APRIL','MAY','JUNE','JULY','AUGUST','SEPTEMBER','OCTOBER','NOVEMBER','DECEMBER'];
    var months=(monthlyData.months||[]).filter(function(m){return ALL_MONTHS.indexOf(String(m).trim().toUpperCase())>=0;});

    // Build national data per month
    var NAT={};
    months.forEach(function(m){
      var r=rows.find(function(x){return (x.Plant||'').toUpperCase()==='NATIONAL'&&String(x.MONTH||x.Month||'').trim().toUpperCase()===m.toUpperCase();});
      if(!r)return;
      function pv(v){return v?(Math.abs(v)<2?v*100:v):0;}
      NAT[m]={
        output:+(r['Total Plant Output,mt']||r['Total Plant Output,mt w/o toll']||0),
        capUtil:+pv(r['Capacity Utilization Rate,%']).toFixed(1),
        oee:+pv(r['OEE']).toFixed(1),
        udt:+(r['Unscheduled Down Time, hr']||0),
        sdt:+(r['Scheduled Down Time, hr']||0),
        kwh:+(r['kWh/ton']||0),
        fuel:+(r['Li/ton']||0),
        coal:+(r['kg/ton']||0),
        rmVar:+(pv(r['RM Variance, %'])).toFixed(3),
    ...
    var oeeV=oeeArr.filter(function(v){return v;});
    var avgOEE=oeeV.length?oeeV.reduce(function(a,v){return a+v;},0)/oeeV.length:0;
    var cuV=cuArr.filter(function(v){return v;});
    var avgCU=cuV.length?cuV.reduce(function(a,v){return a+v;},0)/cuV.length:0;
    var kwhV=kwhArr.filter(function(v){return v;});
    var avgKwh=kwhV.length?kwhV.reduce(function(a,v){return a+v;},0)/kwhV.length:0;
    var fuelV=fuelArr.filter(function(v){return v;});
    var avgFuel=fuelV.length?fuelV.reduce(function(a,v){return a+v;},0)/fuelV.length:0;
    var coalV=coalArr.filter(function(v){return v&&v>0;});
    var avgCoal=coalV.length?coalV.reduce(function(a,v){return a+v;},0)/coalV.length:0;
    var rejV=rejArr.filter(function(v){return v;});
    var avgRej=rejV.length?rejV.reduce(function(a,v){return a+v;},0)/rejV.length:0;
    var maxRej=rejV.length?Math.max.apply(null,rejV):0;
    var minRej=rejV.length?Math.min.apply(null,rejV):0;

    // Latest full month data
    var latestM=months.filter(function(m){return NAT[m]&&NAT[m].output>0;}).slice(-1)[0]||'MAY';
    var MAY=NAT[latestM]||NAT['MAY']||{};
    var mayCost=MAY_COST;
    var tc=mayCost.totalCost||27515051, fc=mayCost.fixedCost||0, vc=mayCost.varCost||0, ct=mayCost.costTon||1676;
    var fixPct=tc>0?Math.round(fc/tc*100):54, varPct=100-fixPct;
    var mayRmv=MAY.rmVar||0;

    // UDT pareto from latest month
    var udtByCat=NAT[latestM]||{};
    var pArr=[
      {cat:'Mechanical',hrs:udtByCat.mech||0},
      {cat:'Warehouse',hrs:udtByCat.warehouse||0},
      {cat:'Change Die',hrs:udtByCat.changeDie||0},
      {cat:'Change Over',hrs:udtByCat.changeOver||0},
      {cat:'Process',hrs:udtByCat.process||0},
      {cat:'Electrical',hrs:udtByCat.electrical||0},
      {cat:'Change Screen',hrs:udtByCat.changeScreen||0},
      {cat:'Raw Materials',hrs:udtByCat.rawMat||0},
    ...
    await buildMonthlyReportPptx({
      months:months,lbls:lbls,NAT:NAT,MAY:MAY,mayCost:mayCost,latestM:latestM,
      outArr:outArr,oeeArr:oeeArr,cuArr:cuArr,kwhArr:kwhArr,fuelArr:fuelArr,
      coalArr:coalArr,costArr:costArr,rejArr:rejArr,rmVarArr:rmVarArr,
      ytdOut:ytdOut,avgOEE:avgOEE,avgCU:avgCU,avgKwh:avgKwh,avgFuel:avgFuel,
      avgCoal:avgCoal,avgRej:avgRej,maxRej:maxRej,minRej:minRej,
      tc:tc,fc:fc,vc:vc,ct:ct,fixPct:fixPct,varPct:varPct,mayRmv:mayRmv,
      pArr:pArr,pHrs:pHrs,pTotal:pTotal,pCum:pCum,top2:top2,top2pct:top2pct,
      PC:PC,TOP10:TOP10,TOP5:TOP5,REJ:REJ,tot10:tot10,
    });
  }catch(e){alert('Error: '+e.message);console.error(e);}
  finally{btn.innerHTML=origText;btn.disabled=false;}
}

async function buildMonthlyReportPptx(d){
  var pres=new PptxGenJS();
  pres.layout='LAYOUT_WIDE';
  var months=d.months,lbls=d.lbls,NAT=d.NAT,MAY=d.MAY,mayCost=d.mayCost,latestM=d.latestM;
  var outArr=d.outArr,oeeArr=d.oeeArr,cuArr=d.cuArr,kwhArr=d.kwhArr,fuelArr=d.fuelArr;
  var coalArr=d.coalArr,costArr=d.costArr,rejArr=d.rejArr,rmVarArr=d.rmVarArr;
  var ytdOut=d.ytdOut,avgOEE=d.avgOEE,avgCU=d.avgCU,avgKwh=d.avgKwh,avgFuel=d.avgFuel;
  var avgCoal=d.avgCoal,avgRej=d.avgRej,maxRej=d.maxRej,minRej=d.minRej;
  var tc=d.tc,fc=d.fc,vc=d.vc,ct=d.ct,fixPct=d.fixPct,varPct=d.varPct,mayRmv=d.mayRmv;
  var pArr=d.pArr,pHrs=d.pHrs,pTotal=d.pTotal,pCum=d.pCum,top2=d.top2,top2pct=d.top2pct;
  var PC=d.PC,TOP10=d.TOP10,TOP5=d.TOP5,REJ=d.REJ,tot10=d.tot10;
  var MON=months;

  var NAVY='0A1628',BLUE='1B4F8C',DBLUE='0D2545',SKY='2979C8',WHITE='FFFFFF';
  var RED='B71C1C',LRED='EF9A9A',GREEN='1B5E20',LGRN='4CAF50';
  var AMBER='C06000',GOLD='F4A300',TEAL='00838F',CYAN='00BCD4';
  var LGRAY='90A4AE';

  function fN(v,d){d=d||0;if(v===null||v===undefined)return '\u2014';return Number(v).toLocaleString('en-PH',{minimumFractionDigits:d,maximumFractionDigits:d});}
  function fM(v){if(!v||v===0)return '\u20b10';return v>=1000000?'\u20b1'+(v/1000000).toFixed(2)+'M':v>=1000?'\u20b1'+(v/1000).toFixed(1)+'K':'\u20b1'+fN(v);}
  function addBg(s){
    s.background={color:NAVY};
  ...
  function kBox(s,x,y,w,h,lbl,val,col){
    s.addShape(pres.ShapeType.roundRect,{x,y,w,h,rectRadius:0.08,fill:{color:DBLUE},line:{pt:1.5,color:col}});
    s.addShape(pres.ShapeType.rect,{x,y,w,h:0.045,fill:{color:col},line:{type:'none'}});
...
{var s=pres.addSlide();
 ...
 hn.forEach(function(h,i){
   var x=0.8+i*4.1;
   s.addShape(pres.ShapeType.roundRect,{x,y:3.82,w:3.7,h:1.32,rectRadius:0.1,fill:{color:DBLUE},line:{pt:1.5,color:h.c}});
   s.addShape(pres.ShapeType.rect,{x,y:3.82,w:3.7,h:0.05,fill:{color:h.c},line:{type:'none'}});
...
{var s=pres.addSlide();
 ...
 var kw=2.95,kg=0.17,ky=0.98;
 kBox(s,0.5,ky,kw,1.0,'YTD OUTPUT (JAN–JUN)',fN(Math.round(ytdOut),0)+' MT',LGRN);
 kBox(s,0.5+kw+kg,ky,kw,1.0,'MAY OUTPUT (LATEST)',fN(MAY.output,0)+' MT',SKY);
 kBox(s,0.5+2*(kw+kg),ky,kw,1.0,'AVG CAP. UTILIZATION',avgCU.toFixed(1)+'%',Number(avgCU.toFixed(1))>=80?LGRN:AMBER);
 kBox(s,0.5+3*(kw+kg),ky,kw,1.0,'AVG OEE',avgOEE.toFixed(1)+'%',Number(avgOEE.toFixed(1))>=85?LGRN:AMBER);

 // Monthly output bar
 s.addChart(pres.charts.BAR,[{name:'Output (MT)',labels:lbls,values:outArr}],{
   x:0.5,y:2.12,w:6.0,h:3.32,barDir:'col',
   showTitle:true,title:'Monthly National Output (MT)',titleFontSize:10,titleColor:WHITE,
 ...
 addBox(s,5.54,0.92,'Analysis:  YTD national output of '+fN(Math.round(ytdOut),0)+' MT across 7 plants through June 2026. MAY was the strongest month at '+fN(MAY.output,0)+' MT — buoyed by full utilization at AC (93%) and HOREB (90%). Average OEE of '+avgOEE.toFixed(1)+'% reflects sound operational efficiency across most plants, though ARGAO (57.5%) and CCPC (14%) continue to underperform against the 80% utilization target. February and April recorded lower output due to scheduled maintenance and planned shutdown windows. June output is partial. PFMIS reported 15% utilization — tolling arrangement limitation, not a plant reliability issue. AC and BUKID remain the primary volume backbone of national supply, together contributing over 58% of YTD output. Sustained utilization improvement at ARGAO and SOUTH is the single biggest lever to raise national output in H2 2026.');
...
{var s=pres.addSlide();
 ...
 // 4 KPI boxes
 var kw=2.95,kg=0.17,ky=0.98;
 var avgKwh=(kwhArr.filter(v=>v).reduce((a,v)=>a+v,0)/kwhArr.filter(v=>v).length);
 var avgFuel=(fuelArr.filter(v=>v).reduce((a,v)=>a+v,0)/fuelArr.filter(v=>v).length);
 var avgCoal=(coalArr.filter(v=>v&&v>0).reduce((a,v)=>a+v,0)/(coalArr.filter(v=>v&&v>0).length||1));
 var mayRmv=MAY.rmVar||0;
 kBox(s,0.5,ky,kw,1.0,'AVG POWER (JAN–JUN)',avgKwh.toFixed(2)+' kWh/ton',avgKwh<=35?LGRN:AMBER);
 kBox(s,0.5+kw+kg,ky,kw,1.0,'AVG FUEL (JAN–JUN)',avgFuel.toFixed(2)+' L/ton',avgFuel<=3.5?LGRN:AMBER);
 kBox(s,0.5+2*(kw+kg),ky,kw,1.0,'AVG COAL (JAN–JUN)',avgCoal>0?avgCoal.toFixed(2)+' kg/ton':'N/A',LGRAY);
 kBox(s,0.5+3*(kw+kg),ky,kw,1.0,'MAY RM VARIANCE',mayRmv>=0?'+'+mayRmv.toFixed(2)+'%':mayRmv.toFixed(2)+'%',mayRmv>=0?LGRN:AMBER);

 // Power + Fuel trend (dual line — left chart)
 s.addChart(pres.charts.LINE,[
 ...
 // Coal + RM Var bar
 var rmVarArr=MON.map(m=>NAT[m]?NAT[m].rmVar:null);
 s.addChart(pres.charts.BAR,[
   {name:'Coal kg/ton',labels:lbls,values:coalArr},
 ...
 s.addChart(pres.charts.LINE,[
   {name:'RM Variance %',labels:lbls,values:rmVarArr},
 ...
 addBox(s,5.54,0.92,'Analysis:  Average power intensity of '+avgKwh.toFixed(2)+' kWh/ton tracks within the 35 kWh/ton limit for all months, reflecting consistent energy discipline. Fuel intensity at '+avgFuel.toFixed(2)+' L/ton average is also within the 3.5 L/ton limit — January was highest at 3.33 L/ton, trending downward by May to 3.39 L/ton. Coal usage (kg/ton) is applicable only to HOREB and ARGAO pelleting lines; national average is suppressed by non-coal plants. RM Variance has been consistently negative (under plan) across all months — indicating material yield below spec. This pattern suggests ongoing need to review raw material quality on receipt. May RM Variance of '+mayRmv.toFixed(2)+'% requires continued attention; tighter incoming quality checks and supplier performance review are recommended for H2 2026.');
...
{var s=pres.addSlide();
 ...
 var avgRej=(rejArr.filter(v=>v).reduce((a,v)=>a+v,0)/rejArr.filter(v=>v).length);
 var maxRej=Math.max(...rejArr.filter(v=>v));
 var minRej=Math.min(...rejArr.filter(v=>v));
 var kw=2.95,kg=0.17,ky=0.98;
 kBox(s,0.5,ky,kw,1.0,'AVG REJECTION RATE',avgRej.toFixed(2)+'%',RED);
 kBox(s,0.5+kw+kg,ky,kw,1.0,'LOWEST (BEST MONTH)',minRej.toFixed(2)+'%',LGRN);
 kBox(s,0.5+2*(kw+kg),ky,kw,1.0,'HIGHEST MONTH',maxRej.toFixed(2)+'%',AMBER);
 kBox(s,0.5+3*(kw+kg),ky,kw,1.0,'1% THRESHOLD STATUS',avgRej<=1?'WITHIN LIMIT':'BREACHED',avgRej<=1?LGRN:RED);
 ...
 addBox(s,5.54,0.92,'Analysis:  National rejection rate has remained within the 1% quality threshold throughout January–June 2026, averaging '+avgRej.toFixed(2)+'%. February recorded the best rate at '+minRej.toFixed(2)+'% — attributed to die maintenance at HOREB and AC completed in January. March spiked to '+maxRej.toFixed(2)+'% due to raw material quality issues from a new supplier batch at BUKID. SOUTH (2.32%) and ARGAO (1.12%) are chronic above-threshold contributors in May — both sites require mandatory die-inspection protocols and root cause reports. AC, HOREB, BUKID, and PFMIS all perform below 0.6% — these are the quality-reliable plants for high-value customer orders. Outright reject (non-recoverable) averages 0.25% nationally, while Other Reject (remill-recoverable) is 0.40% — proactive remill scheduling is essential to minimize throughput impact from recoverable rejects.');
...
{var s=pres.addSlide();
 ...
 var kw=3.88,kg=0.18,ky=0.98;
 kBox(s,0.5,ky,kw,1.0,'MAY TOTAL COST',fM(tc),RED);
 kBox(s,0.5+kw+kg,ky,kw,1.0,'COST PER TON','₱'+fN(Math.round(ct),0),SKY);
 kBox(s,0.5+2*(kw+kg),ky,kw,1.0,'FIXED vs VARIABLE',fixPct+'% / '+varPct+'%',AMBER);

 // Cost per ton trend line (main chart)
 ...
 // Cost breakdown mini donut-like — use bar
 s.addChart(pres.charts.BAR,[
   {name:'Cost Component',labels:['Rental/Amor','Power','Agency MP','Spareparts','Diesel','Other'],values:[9802488,6422934,3506984,1279494,3829004,2673147]},
 ...
 addBox(s,5.54,0.92,'Analysis:  MAY production cost of '+fM(tc)+' at ₱'+fN(Math.round(ct),0)+'/ton represents the lowest cost month YTD — supported by the highest output volume ('+fN(MAY.output,0)+' MT) which diluted fixed overheads. The ₱/ton trend shows improvement from the February peak of ₱1,921/ton, confirming that volume is the primary lever for cost reduction. Rental/amortization (₱9.8M, largest component) is a fixed obligation and cannot be reduced short-term — making sustained volume utilization the most actionable path to cost competitiveness. Power (₱6.4M) and Diesel (₱3.8M) are the primary controllable variable costs; energy conservation programs at HOREB and BUKID would have the highest ROI. Agency manpower (₱3.5M) should be reviewed for optimization particularly at sites operating below 70% utilization, where the fixed labor component creates disproportionate ₱/ton amplification.');
...
{var s=pres.addSlide();
 ...
 var hRow=[
   ...
   th('CESS Dep.'),th('SP Dep.'),th('Insur.'),th('Water'),th('Thread'),
   th('Total Cost'),th('₱/kg'),
 ];
 var tData=[hRow];
 PC.forEach(function(r,i){
   var bg=i%2===0?'0E2040':'0A1628';
   var isNat=r.plant==='NATIONAL';
   var fill=isNat?'0D3060':bg;
   var tc2=isNat?WHITE:'B0CCEC';
   var f=function(v){return v>0?fN(v,0):'—';};
   tData.push([
     td(r.plant,fill,isNat?'4FC3F7':SKY,true,'left'),
     td(fN(r.vol_kg,0),fill,tc2),
     td(f(r.rental),fill,tc2),td(f(r.spares),fill,tc2),td(f(r.mfgdirect),fill,tc2),
     td(f(r.diesel),fill,tc2),td(f(r.elec),fill,tc2),td(f(r.agency),fill,tc2),
     td(f(r.other),fill,tc2),td(f(r.third),fill,tc2),
     td(f(r.cess),fill,tc2),td(f(r.sp),fill,tc2),td(f(r.ins),fill,tc2),
     td(f(r.water),fill,tc2),td(f(r.thread),fill,tc2),
 ...
 addBox(s,4.4,1.1,'Analysis:  NATIONAL weighted ₱/kg of ₱1.66 reflects blended cost across all plants including MG (mixing/grinding) lines. AC (₱1.31/kg) is the most cost-efficient pelleting plant, driven by highest throughput (5.11M kg) diluting the ₱1.8M rental burden. BUKID (₱1.59/kg) benefits from high volume (5.34M kg) despite carrying the heaviest rental cost at ₱3.02M. ARGAO (₱2.32/kg) reflects the cost impact of underutilization — high fixed rental (₱2.24M) spread over only 2.12M kg of output creates a ₱0.73/kg premium above the national average. PFMIS (₱3.20/kg) is the highest-cost site, driven almost entirely by fixed rental with very low volume — a tolling arrangement structure limits throughput scale. HOREB MG (₱0.98/kg) and AC MG (₱0.94/kg) are cost-efficient mixing operations with minimal fixed overhead, largely driven by agency manpower and electricity. Diesel cost at HOREB (₱1.05M) and BUKID (₱1.49M) are the largest fuel components — energy conservation programs at these two sites would yield the highest national cost reduction impact.');
...
{var s=pres.addSlide();
 ...
 // DT scorecards
 var dtCards=[
   {l:'SCHED. DT',v:(MAY.sdt||0).toFixed(1)+' hrs',col:LGRAY},
   {l:'UNSCHED. DT',v:(MAY.udt||0).toFixed(1)+' hrs',col:MAY.udt>200?RED:AMBER},
   {l:'MECHANICAL',v:(MAY.mech||0).toFixed(1)+' hrs',col:RED},
   {l:'WAREHOUSE',v:(MAY.warehouse||0).toFixed(1)+' hrs',col:AMBER},
   {l:'CHANGE DIE',v:(MAY.changeDie||0).toFixed(1)+' hrs',col:AMBER},
   {l:'CHANGE OVER',v:(MAY.changeOver||0).toFixed(1)+' hrs',col:AMBER},
   {l:'PROCESS',v:(MAY.process||0).toFixed(1)+' hrs',col:AMBER},
   {l:'ELECTRICAL',v:(MAY.electrical||0).toFixed(1)+' hrs',col:LGRAY},
   {l:'CHANGE SCREEN',v:(MAY.changeScreen||0).toFixed(1)+' hrs',col:LGRAY},
   {l:'RAW MATERIALS',v:(MAY.rawMat||0).toFixed(1)+' hrs',col:LGRAY},
 ];
 var dcW=1.26,dcH=0.78,dcG=0.05,dcY=1.0;
 dtCards.forEach(function(c,i){
   var x=0.5+i*(dcW+dcG);
 ...
 s.addChart(
   [{type:pres.charts.BAR,data:[{name:'UDT (hrs)',labels:pArr.map(function(r){return r.cat;}),values:pHrs}],options:{barDir:'col',chartColors:['B71C1C']}},
    {type:pres.charts.LINE,data:[{name:'Cumulative %',labels:pArr.map(function(r){return r.cat;}),values:pCum}],options:{chartColors:[SKY],lineSize:2,lineDataSymbol:'circle',lineDataSymbolSize:5,secondaryValAxis:true,secondaryCatAxis:true}}],
   {x:0.5,y:1.92,w:7.8,h:3.38,showTitle:true,title:'UDT Pareto by Category — MAY 2026 National',titleFontSize:10,titleColor:WHITE,
 ...
 var ptRows=[[th('#','center'),th('CATEGORY','left'),th('HRS'),th('%'),th('CUM%')]];
 var cumP=0;
 pArr.forEach(function(r,i){
   var pct=(r.hrs/pTotal)*100; cumP+=pct;
   var bg=i%2===0?'0E2040':'0A1628';
   ptRows.push([td(String(i+1),bg,LGRAY,false,'center'),td(r.cat,bg,cumP<=80+pct?'90CAF9':'B0CCEC',cumP<=80+pct,'left'),td(r.hrs.toFixed(1),bg,LRED,true),td(pct.toFixed(1)+'%',bg,WHITE),td(cumP.toFixed(1)+'%',bg,cumP>=80?AMBER:LGRN,cumP>=80)]);
 });
 ...
 var top2=pArr.slice(0,2);
 var top2pct=top2.reduce(function(a,r){return a+r.hrs;},0)/pTotal*100;
 addBox(s,5.42,0.92,'Analysis:  Total unscheduled downtime for MAY was '+MAY.udt.toFixed(1)+' hrs across all national plants. Mechanical failures ('+MAY.mech.toFixed(1)+' hrs, '+(MAY.mech/MAY.udt*100).toFixed(0)+'%) dominate the downtime profile — primarily bucket elevator issues at HOREB and pellet mill failures at BUKID and ARGAO. Mechanical and Warehouse together account for '+top2pct.toFixed(0)+'% of all UDT — focused PM on these two categories would recover the majority of lost production time. The 80% cumulative threshold is reached after the 3rd category (Change Die), meaning the top 3 categories (Mechanical, Warehouse, Change Die) are the highest-leverage targets for downtime reduction. Scheduling preventive maintenance windows for bucket elevators, pellet mill rolls, and ring die replacement during planned downtime periods would significantly reduce emergency breakdowns. Change Die ('+MAY.changeDie.toFixed(1)+' hrs) and Change Over ('+MAY.changeOver.toFixed(1)+' hrs) suggest die lifecycle management improvements are needed at HOREB and ARGAO.');
 s.addText('VPI Operations  ·  Downtime Analysis — MAY 2026',{x:0.5,y:7.22,w:12.33,h:0.22,fontFace:'Calibri',fontSize:8.5,color:'607D8B',margin:0});}
...
{var s=pres.addSlide();
 ...
 // Top 5 per site table (right)
 var SITE_COLORS={'AC':SKY,'HOREB':RED,'ARGAO':AMBER,'PFMIS':TEAL,'BUKID':'7B2D8B'};
 ...
 SITES.forEach(function(site,si){
   ...
   entries.forEach(function(r,i){
     ...
     tRows.push([td(i===0?site:'',bg,sc,i===0,'left'),td(String(r.rank),bg,LGRAY,false,'center'),td(r.sub,bg,r.rank===1?WHITE:'B0CCEC',r.rank===1,'left'),td(r.hrs.toFixed(1),bg,r.rank===1?LRED:LGRAY,r.rank===1)]);
   });
 });
...
{var s=pres.addSlide();
 ...
 // 6 KPI summary callouts (2 rows of 3)
 var sumCards=[
   {l:'YTD OUTPUT',v:fN(Math.round(ytdOut),0)+' MT',s:'vs plan — supply confirmed',col:LGRN},
   {l:'MAY OEE',v:MAY.oee.toFixed(1)+'%',s:MAY.udt.toFixed(0)+' hrs UDT | SDT '+MAY.sdt.toFixed(0)+' hrs',col:MAY.oee>=85?LGRN:AMBER},
   {l:'COST/TON (MAY)',v:'₱'+fN(Math.round(mayCost.costTon||1676),0),s:'Total '+fM(mayCost.totalCost||27515051),col:GOLD},
   {l:'AVG REJECTION',v:((rejArr.filter(v=>v).reduce((a,v)=>a+v,0)/rejArr.filter(v=>v).length)).toFixed(2)+'%',s:'Within 1% threshold YTD',col:LGRN},
   {l:'AVG POWER',v:(kwhArr.filter(v=>v).reduce((a,v)=>a+v,0)/kwhArr.filter(v=>v).length).toFixed(2)+' kWh/t',s:'Within 35 kWh/ton limit',col:LGRN},
   {l:'RM VARIANCE',v:(MAY.rmVar>=0?'+':'')+MAY.rmVar.toFixed(2)+'%',s:'Material yield monitoring active',col:MAY.rmVar>=0?LGRN:AMBER},
 ];
 var bw=3.82,bhh=1.08,bgap=0.18,bx0=0.62;
 sumCards.forEach(function(c,i){
   ...
   s.addText(c.s,{x:x+0.18,y:y+0.76,w:bw-0.36,h:0.22,fontFace:'Calibri',fontSize:8,color:'7FA8CC',margin:0});
 });
 ...
 var priorities=[
   {txt:'🔴  MECHANICAL PM ESCALATION: HOREB Bucket Elevator (22.4 hrs) and BUKID Pellet Mill (16.2 hrs) are the top-2 national downtime contributors. Site Managers must complete PM within 2 weeks. Target: ZERO recurrence of same failure mode in June. Failure to complete will trigger reliability KPI penalty.',ok:false,col:'7F0000'},
   {txt:'🟠  ARGAO RECOVERY PLAN DUE: Utilization 57.5%, OEE 69.3%, Rejection 1.12%, Cost ₱2.32/kg. Site Manager to submit formal June recovery plan covering: production schedule ramp-up, die inspection protocol, pellet mill maintenance, and Change Over reduction to below 15 hrs. Review at next COMEX.',ok:false,col:RED},
   {txt:'🟡  DIE LIFECYCLE PROGRAM: Change Die and Change Over together account for 55+ hrs of national UDT monthly. Implement a national die lifecycle tracking system — schedule replacements proactively during planned DT windows instead of reactive emergency change-outs. Target: 30% reduction in unplanned die-related downtime by July.',ok:false,col:AMBER},
   {txt:'🟢  QUALITY GATE MAINTAIN: National rejection rate maintained within 1% threshold YTD (avg '+((rejArr.filter(v=>v).reduce((a,v)=>a+v,0)/rejArr.filter(v=>v).length)).toFixed(2)+'%). SOUTH (2.32%) and ARGAO (1.12%) must submit root cause reports and monthly corrective action updates. Sustain die-inspection protocol at all sites. No relaxation of quality gate despite volume pressure.',ok:true,col:GREEN},
   {txt:'🔵  COST OPTIMIZATION — H2 2026: MAY cost of ₱1,676/ton is the YTD low point. Target ₱1,650/ton for June–August by: (1) maintaining or growing AC/BUKID output volume, (2) reducing agency manpower at under-utilized sites, (3) power factor correction at HOREB to reduce electricity cost per ton, (4) diesel consumption audit at BUKID.',ok:false,col:SKY},
 ];
 priorities.forEach(function(p,i){
   var y=4.02+i*0.61;
   s.addShape(pres.ShapeType.roundRect,{x:0.62,y,w:12.0,h:0.55,rectRadius:0.07,fill:{color:'091B30'},line:{pt:1.2,color:p.col}});
   s.addShape(pres.ShapeType.rect,{x:0.62,y:y+0.07,w:0.06,h:0.41,fill:{color:p.col},line:{type:'none'}});
   s.addText(p.txt,{x:0.8,y,w:11.62,h:0.55,fontFace:'Calibri',fontSize:9.5,color:WHITE,valign:'middle',margin:0,lineSpacingMultiple:1.1});
 });
 s.addText('VPI Operations Dashboard  ·  Monthly COMEX Report  ·  MAY 2026',{x:0.65,y:7.22,w:12,h:0.22,fontFace:'Calibri',fontSize:8.5,color:'607D8B',margin:0});}
  ...
  var a=document.createElement('a');
  a.href=url;
  a.download='VPI_Monthly_Report_'+latestM+'2026.pptx';
