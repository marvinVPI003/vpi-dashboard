...
function renderOEE(){
  ...
  function nv(v){
    ...
    // If value came with % sign it's already a percentage - return as-is
    return hasPct?n:n;
  }
  ...
  function pCol(v){if(v===null||v===undefined)return 'var(--text2)';return v<0?'var(--red)':'var(--green)';}
  ...
  gasGet('variance').then(function(d){
    ...
    var activeSiteV='NATIONAL';

    function getRow(plant,month){
      return rows.find(function(r){
        return (r.Plant||'').toUpperCase()===plant.toUpperCase()&&
               String(r.MONTH||'').trim().toUpperCase()===month.toUpperCase();
      })||null;
    }

    // Scorecard for one row
    function buildSC(r){
      if(!r) return '<div style="color:var(--text3);font-size:10px;padding:10px">No data for this selection</div>';
      ...
      return '<div style="display:grid;grid-template-columns:repeat(5,1fr);gap:8px">'
        +sc('TOTAL INPUT (mt)',fN(ti,1),'','var(--blue)')
        +sc('RM VARIANCE %',fPct(vp),fN(vq,2)+' mt',pCol(vp))
        +sc('RM VARIANCE (₱)',fPhp(vph),'Monetary impact',pCol(vp))
        +sc('ABS RM VARIANCE %',(ap!==null?Math.abs(ap).toFixed(2)+'%':'—'),fN(aq,2)+' mt','var(--amber)')
        +sc('VAR % w/o SACKS',fPct(wp),fPhp(wph),pCol(wp))
        +'</div>';
    }

    // Site pills
    var sitesInData=[];
    SITES_ORDER.forEach(function(s){
      if(rows.some(function(r){return (r.Plant||'').toUpperCase()===s;})) sitesInData.push(s);
    });

    function buildSitePills(activeSite){
      return '<div style="display:flex;gap:5px;flex-wrap:wrap;margin-bottom:8px">'
        +sitesInData.map(function(s){
          var act=s===activeSite;
          return '<button onclick="window._varSite(\''+s+'\')" id="vsite-'+s+'" '
            +'style="padding:3px 11px;border-radius:12px;border:1px solid '+(act?'var(--blue)':'var(--border)')+';'
            +'background:'+(act?'var(--blue)':'transparent')+';color:'+(act?'#fff':'var(--text2)')+';'
            +'font-size:9px;cursor:pointer;font-weight:'+(act?'700':'400')+'">'+s+'</button>';
        }).join('')+'</div>';
    }

    // Month pills
    function buildMonthPills(activeM){
      return '<div style="display:flex;gap:5px;flex-wrap:wrap;margin-bottom:12px">'
        +months.map(function(m){
          var short=m.charAt(0)+m.slice(1,3).toLowerCase();
          var act=m.toUpperCase()===activeM.toUpperCase();
          return '<button onclick="window._varMonth(\''+m+'\')" id="vpill-'+m+'" '
            +'style="padding:3px 11px;border-radius:12px;border:1px solid '+(act?'var(--sky)':'var(--border)')+';'
            +'background:'+(act?'var(--sky)':'transparent')+';color:'+(act?'#fff':'var(--text2)')+';'
            +'font-size:9px;cursor:pointer;font-weight:'+(act?'700':'400')+'">'+short+'</button>';
        }).join('')
        +'<button onclick="window._varMonth(\'ALL\')" id="vpill-ALL" '
        +'style="padding:3px 11px;border-radius:12px;border:1px solid var(--border);background:transparent;color:var(--text2);font-size:9px;cursor:pointer">ALL</button>'
        +'</div>';
    }
    ...
    var TD='padding:5px 8px;font-size:9px;border-bottom:1px solid var(--border);text-align:right;white-space:nowrap;';
    ...
    function buildTable(filterM){
      ...
      filtered.sort(function(a,b){
        ...
        var ia=SITES_ORDER.indexOf(pa), ib=SITES_ORDER.indexOf(pb);
        return (ia<0?99:ia)-(ib<0?99:ib);
      });

      if(!filtered.length) return '<div style="color:var(--text3);padding:20px;text-align:center">No data</div>';

      var html='<div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:9px"><thead><tr>'
      ...
      filtered.forEach(function(r,i){
        ...
        var rowBg=isNat?'var(--navy)':(i%2===0?'var(--bg1)':'var(--bg2)');
        var vp=nv(r['RM Variance, %']);
        var wp=nv(r['RM Variance (w/o used sacks), %']);
        var ap2=nv(r['ABS RM Variance, %']);
        var vpc=pCol(vp), wpc=pCol(wp);
        html+='<tr style="background:'+rowBg+'">'
          ...
          +'<td style="'+TD+'color:'+vpc+'">'+fPhp(nv(r['RM Variance, Php']))+'</td>'
          +'<td style="'+TD+'">'+fN(nv(r['ABS RM Variance, Qty']),2)+'</td>'
          +'<td style="'+TD+'color:var(--amber);font-weight:700">'+(ap2!==null?Math.abs(ap2).toFixed(2)+'%':'—')+'</td>'
          +'<td style="'+TD+'">'+fN(nv(r['RM Variance (w/o used sacks), Qty']),2)+'</td>'
          +'<td style="'+TD+'font-weight:700;color:'+wpc+'">'+fPct(wp)+'</td>'
          +'<td style="'+TD+'color:'+wpc+'">'+fPhp(nv(r['RM Variance (w/o used sacks), Php']))+'</td>'
          +'</tr>';
      });
    ...
    function renderAll(){
      var scRow=getRow(activeSiteV, activeM==='ALL'?months[months.length-1]:activeM);
      document.getElementById('var-sc').innerHTML=buildSC(scRow);
      // Update site pills
      sitesInData.forEach(function(s){
        var b=document.getElementById('vsite-'+s);
        if(!b) return;
        var act=s===activeSiteV;
      ...
      months.forEach(function(m){
        var b=document.getElementById('vpill-'+m);
        if(!b) return;
        var act=m.toUpperCase()===activeM.toUpperCase();
        b.style.background=act?'var(--sky)':'transparent';
        b.style.borderColor=act?'var(--sky)':'var(--border)';
        b.style.color=act?'#fff':'var(--text2)';
        b.style.fontWeight=act?'700':'400';
      });
      var allB=document.getElementById('vpill-ALL');
      if(allB){
        var act=activeM==='ALL';
        allB.style.background=act?'var(--sky)':'transparent';
        allB.style.borderColor=act?'var(--sky)':'var(--border)';
        allB.style.color=act?'#fff':'var(--text2)';
      }
      document.getElementById('var-table').innerHTML=buildTable(activeM);
    }

    window._varSite=function(s){activeSiteV=s;renderAll();};
    window._varMonth=function(m){activeM=m;renderAll();};

    // Initial render
    var monthLabel=activeM.charAt(0)+activeM.slice(1).toLowerCase();
    ct.innerHTML='<div style="padding:12px">'
      ...
      +'<div style="font-size:10px;color:var(--sky);font-weight:700" id="var-label">'+monthLabel+' 2026</div>'
      +'</div>'
      +'<div style="margin-bottom:10px"><div style="font-size:9px;color:var(--text3);margin-bottom:5px;font-weight:700;letter-spacing:.5px">SITE</div>'
      +buildSitePills(activeSiteV)+'</div>'
      +'<div id="var-sc" style="margin-bottom:12px">'+buildSC(getRow(activeSiteV,activeM))+'</div>'
      +'<div style="margin-bottom:6px"><div style="font-size:9px;color:var(--text3);margin-bottom:5px;font-weight:700;letter-spacing:.5px">MONTH FILTER</div>'
      +buildMonthPills(activeM)+'</div>'
      +'<div id="var-table">'+buildTable(activeM)+'</div>'
      +'</div>';
